...
export default function Product() {
  ...
  const updateSelectedFilters = () => {
    ...
    // Iterate over each filter category and update selectedFilters
    filters.forEach((section) => {
      const filterValues = searchParams.getAll(section.id);
      if (filterValues.length > 0) {
        setSelectedFilters((prevFilters) => ({
          ...prevFilters,
          [section.id]: filterValues,
        }));
      }
    });
  ...
  const isFilterOptionSelected = (sectionId, optionValue) => {
    if (selectedFilters[sectionId]) {
      return selectedFilters[sectionId].includes(optionValue);
    }
    return false;
  };
  ...
  const handleFilter = (value, sectionId) => {
    ...
    if (updatedFilters[sectionId].length > 0) {
      searchParams.set(sectionId, updatedFilters[sectionId].join(","));
    } else {
                                ...
                                {section.options.map((option, optionIdx) => (
                                  ...
                                  >
                                    <input
                                      ...
                                      type="checkbox"
                                      defaultChecked={isFilterOptionSelected(
                                        section.id,
                                        option.value
                                      )}
